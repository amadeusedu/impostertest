...
## Supabase Magic Link Redirects (Dev Client + Expo Go)

When using Supabase magic links, add your app redirect URL(s) to **Supabase Auth → Redirect URLs**.
For this app, the redirect path is:

```
<scheme>://auth/callback
```

With the current app scheme (`imposter`), the dev client redirect URL is:

```
imposter://auth/callback
```

If you test with Expo Go, ensure the Expo Go redirect URL generated by `Linking.createURL("auth/callback")` is also whitelisted.
